...
FOR %%b IN (%*) do (
    set "_arg_=%%b"
    if !keep_next!==1 (
        set "_arg_list_=!_arg_list_!;!_arg_!"
        set /a "_cnt+=1+0"
    )
    set keep_next=0
    if !_arg_!==--debugger (
        set "_arg_list_=!_arg_!" 
        set /a "_cnt+=1+0"
    )
...
set "_arg_list_=--x-cmake-args=-DVCPKG_CMAKE_CONFIGURE_OPTIONS=!_arg_list_!"
...
echo args: !_arg_list_!
call !vcpkg_exe! install !ports_to_install! --debug --triplet !triplet! --host-triplet !host_triplet! !more_opts! --editable !_arg_list_!
